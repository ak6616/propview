...
export default function AgentCard({ agent }: { agent: AgentProfile }) {
  return (
    <Link
      ...
      className="flex flex-col items-center rounded-xl border border-slate-200 bg-white p-6 text-center shadow-sm transition hover:shadow-md"
    >
      ...
      <p className="mt-1 text-xs text-slate-500">{agent.listingCount} listings</p>
    </Link>
  );
}
